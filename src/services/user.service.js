...
class UserService {
  ...
  async createUser (data) {
    ...
    if (
      ...
      data.faceDescriptor.length !== 128
    ) {
  ...
  async getAllUsers (page = 1, limit = 10, search = '') {
    const result = await this.UserRepository.getAllUsers(page, limit, search)
    return result
  }

    async getAllStudents () {
    try {
      const students = await this.UserRepository.getAllStudents()
